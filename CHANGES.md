...
### 어떤 모델을 쓸지 실제로 비교했습니다

기준 음식 `구이 · 갈비구이` 에 대한 유사도 상위 항목입니다.

| 방식 | 비슷하다고 본 것 | 유사도 폭 | 평가 |
|---|---|---|---|
| 히스토그램 | 고사리나물, 만두, 미역국 | 0.926~0.902 | 색만 봄. 무의미 |
| 오토인코더 (직접 학습) | 미역줄기볶음, 물냉면, 감자전 | 0.454~0.387 | 학습 부족. 사실상 무작위 |
| **ResNet18 (사전학습)** | **보쌈, 총각김치, 주꾸미볶음** | **0.915~0.891** | **채택** |

오토인코더는 구조를 고쳐 재학습했는데도 416장(음식당 평균 2.8장)으로는 유의미한
임베딩을 얻지 못했습니다. 유사도 0.454~0.387 은 128차원 무작위 벡터끼리도 나올 수
있는 범위입니다.

ResNet18 은 붉은 양념 계열의 고기·해산물을 찾아내 계열 구분은 됩니다. 다만 상위
항목이 사실상 동점이라 변별력은 부족합니다. ImageNet 이 한식 반찬을 구분하도록
학습되지 않았기 때문입니다.

자세한 과정은 [docs/EXPERIMENTS.md](docs/EXPERIMENTS.md).
...
실험 결과 사전학습 ResNet18 을 기본으로 채택했습니다. 직접 학습한 오토인코더도
그대로 지원하므로 데이터를 확충한 뒤 다시 시도할 수 있습니다.
